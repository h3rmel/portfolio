...
<template>
  <Section :title="t('skills.title')" :subtitle="t('skills.subtitle')">
    <!-- Hard Skills -->
    <section class="flex flex-col gap-4 mb-6 sm:mb-8" aria-labelledby="hard-skills-title">
      <!-- Header -->
      <hgroup>
      ...
      <TooltipProvider :delay-duration="100" :skip-delay-duration="0" :disable-closing-trigger="true">
        <ul class="grid grid-cols-3 place-items-center gap-7 md:grid-cols-4 lg:grid-cols-5 sm:gap-16" role="list">
          <li v-for="hardSkill in hardSkillsData" :key="hardSkill.name">
            <Tooltip>
      ...
      <!-- Other Skills List -->
      <section class="flex flex-col gap-2 mt-2" aria-labelledby="other-hard-skills-title">
    ...
    <section class="flex flex-col gap-4" aria-labelledby="soft-skills-title">
      ...
      <ul class="flex flex-col gap-8 sm:grid sm:grid-cols-2" role="list">
        <li v-for="softSkill in softSkillsData" :key="softSkill.name">
          <Card class="glow group p-6 flex flex-col items-center text-center h-full" :visual-effect="true">
            <component
              :is="softSkill.icon"
              :size="48"
